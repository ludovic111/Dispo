...
- Demande sur capture : répondre à une question précise. Action Répondre, appui sur le texte ou appui long sur la bulle ; aperçu annulable au-dessus du champ, citation dans la réponse et ouverture du message d'origine. Le texte saisi est conservé à l'annulation et restauré avec la citation si l'envoi échoue. La réponse envoyée est ramenée dans la zone visible.
- Persistance : `reply_to_id` nullable avec FK, index, contrôle du même groupe et du message non supprimé par trigger privé invoker. Lecture des citations avec la RLS existante, y compris hors de la page chargée ; mises à jour et suppressions synchronisées sans copier le contenu d'origine. Les anciens clients continuent à envoyer sans citation. Aucun changement des conversations privées ou du site.
- Validation : TypeScript, ESLint, 62 suites / 367 tests ; SQL transactionnel local (membre/non-membre, autre groupe, message absent/supprimé, référence immuable, édition et suppression). Quatre parcours XCTest iOS réussis ; parcours Android réel, annulation conservant le texte, réponse à une réponse, réception entre deux comptes et persistance après redémarrage. Lectures API du second compte confirmant les identifiants des messages cités. Captures avec clavier et citations inspectées sur les deux plateformes.
- Migration `20260905123348_group_message_replies` appliquée en production dans le périmètre du push partout déjà demandé ; colonne/RLS/trigger/index/grants/invoker/historique contrôlés. Le défaut de cache pg-delta du CLI n'a pas empêché son application. Advisors : 52 avertissements RPC definer préexistants, trois informations sécurité ; 33 informations performance dont l'index de réponses encore inutilisé avant livraison. Aucun nouvel avertissement.
- Deux comptes QA locaux révoqués/supprimés avec leur groupe et messages, absence contrôlée. Configuration production restaurée à l'octet près ; Metro et relais local arrêtés. Le lien de session automatique Android n'a pas établi la session ; connexion native par formulaire réussie, sans modification du code d'authentification. Préférence de debug locale configurée pour Metro.
- Expo Doctor 20/21, 14 écarts patch préexistants conservés hors scope. Preuves dans `Dispo-dist/qa-20260905-message-replies/`. Aucun appareil physique testé. Builds de distribution et verdict Apple complétés après exécution.
...
- Périmètre confirmé par Ludovic après les trois captures : le leader ajoute un membre sans compte directement au groupe ; la cloche suit la lecture des conversations ; les intitulés des onglets du groupe restent complets ; le flux SOS conserve uniquement SOS / Mes SOS.
- Membres : formulaire replié par défaut, nom, rôle et statut permanent/guest ; modification et exclusion réservées au leader. Aucune création de profil ou de compte Auth. Le membre apparaît dans la liste partagée et les choix de solos existants. Son exclusion retire ses solos du répertoire et des événements sans altérer les autres affectations.
- Notifications : acquittement des seules notifications de messages du fil visible, jusqu'à l'horodatage serveur du dernier message chargé ; focus et premier plan requis. Les insertions tardives et reconnexions sont traitées ; les autres fils, événements, invitations et SOS restent non lus. Une erreur conserve l'état non lu.
- Onglets : largeur selon le libellé, défilement horizontal si nécessaire. SOS : retrait des sous-filtres Pour moi / École / Tout et du sélecteur d'écoles du flux ; les formulaires de publication et leurs écoles restent inchangés.
- Backend : migrations `20260907100846_group_manual_members` et `20260907101013_read_thread_notifications` appliquées et testées uniquement en local. RLS membre/leader, isolation entre groupes/utilisateurs, limites de saisie, absence de compte fictif, solos mixtes et nettoyage, acquittement borné et alertes conservées couverts par deux suites SQL transactionnelles. Advisors locaux : « No issues found » ; aucun advisor distant ni déploiement production.
- Validation applicative : TypeScript, ESLint, 65 suites / 372 tests réussis. CNG production et pods régénérés. Expo Doctor 20/21 : les 14 écarts patch préexistants restent hors périmètre, aucune dépendance modifiée.
- Validation native : iPhone 17 / iOS 26.4, ajout avec clavier ouvert, liste du second compte, affectation d'un solo puis enregistrement/réouverture, SOS / Mes SOS ; quatre XCTest de parcours réussis. Android 36 medium_phone : connexion par formulaire, membre visible sans droits leader, solo créé sur iOS retrouvé sur Android, onglets, SOS / My SOS. Petit format 360×640 et texte à 130 % : libellés accessibles par défilement, captures inspectées. Le lien de session Android n'a pas établi la session ; le formulaire natif a fonctionné sans modification de l'authentification.
- Persistance relue par les deux comptes authentifiés. Cloche iOS : deux alertes initiales, une seule après lecture du message ; l'alerte d'événement reste non lue. Deux comptes QA révoqués/supprimés avec leur groupe, absence contrôlée. Configuration production restaurée à l'octet près, identifiants locaux temporaires retirés et Metro arrêté.
- Preuves : `Dispo-dist/qa-20260907-group-members/`. Aucun appareil physique testé. **TestFlight explicitement différé**, version/build conservés à 2.4 (49). Les deux migrations devront être approuvées et appliquées au backend existant avant la future livraison de ce code. Aucun App Review, Google Play, compte ou secret de production modifié.
...
- Chargement/erreur des groupes distingués de l’état vide, données en cache préservées et reprise proposée. Boutons avec état sélectionné accessible, noms de groupes complets et retours à la ligne adaptés au texte agrandi. Cinq nouvelles traductions dans les neuf langues.
- Validation : TypeScript, ESLint, formats et 66 suites / 375 tests. Deux parcours XCTest iOS réussis avec composants réels ; vérifications Android des périodes et de la séparation, thèmes sombre/clair, état sans groupe, noms longs, petit format 360×640 à 130 %. Banc sans compte ni backend, retiré avant compilation de production ; package.json restauré à l’octet près et configuration production inchangée. Preuves : `Dispo-dist/qa-20260907-home/REPORT.md`. Aucun appareil physique testé.
- Sur autorisation explicite de Ludovic, migrations du lot précédent `20260907100846_group_manual_members` et `20260907101013_read_thread_notifications` appliquées au backend existant. Historique 84 migrations, RLS/policies/grants/RPC invoker/realtime/trigger vérifiés ; advisors sans nouveau warning sur le lot. L’index neuf encore inutilisé est une information attendue.
- TestFlight réautorisé explicitement (« quand t’as fini push sur TestFlight », puis « push tout »). Build incrémenté à 50 ; CNG et pods production réussis, Expo Doctor 20/21 avec 14 écarts patch préexistants. Verdicts des builds et d’Apple ajoutés après exécution.

- Build Android Release arm64 50 réussi, APK signé v2 vérifié, installé et démarré à froid. Endpoint production et fonctionnalités du lot présents dans le bundle, montage QA absent. Émulateur et Gradle arrêtés après contrôle. iOS Release et import Apple encore en cours à ce stade.
...
## 2026-09-08 — Profils, répertoire et abonnements — préparation build 55

- Suite du build54 importé par Apple : onglets Profil / Mon répertoire / Disponibilités, démos dans Profil et voyages dans Disponibilités. Cartes uniformes avec pastilles réservées à l’école et à la relation ; instruments/niveaux sur une ligne.
- Répertoire personnel et groupes partagent le catalogue, les pochettes et le panneau des informations musicales. Arrangements personnels séparés des morceaux du groupe ; maîtrise, confidentialité, exclusions persistantes et copie vers groupes/événements. Enrichissement partiel corrigé : une métadonnée absente ne supprime plus l’album ou les liens déjà connus. Les extraits iTunes ne sont plus téléchargés automatiquement pour analyse, conformément aux conditions de leur API ; tonalité/BPM issus des métadonnées et saisie manuelle conservés.
- Groupe : un groupe dirigé. Premium : groupes illimités, répertoire personnel et fonctionnalités avancées. SDK RevenueCat, achat/restauration StoreKit, prix localisés Apple, codes écoles et gestion de l’abonnement. Droits calculés côté serveur avec échéance, verrou de concurrence et protection des champs client. Activation production à coordonner avec la livraison du nouveau client.
- Préparation Apple : consentement18+, EULA et règles communautaires, géolocalisation uniquement sur action explicite, confidentialité16types, suppression de compte et procédure Apple de révocation. Filtrage de texte public et erreurs explicites, sans filtrage automatique prétendu des images ou messages privés.
- QA exécutée : TypeScript/ESLint et390tests/71suites ; ExpoDoctor21/21 ; CNG/pods, builds natifs ReleaseiOS etAndroid et tests unitaires Gradle. SQLlocal arrangements/RLS/abonnements/modération, créationGroupe concurrente limitée à1. iPhone : édition personnelle sans modifier le groupe, maîtrise/partage/filtre/copie événement, cartes avec école/relation, thèmesFRsombre/ENclair, premier groupe autorisé et second verrouillé. Bug découvert et corrigé : changement de compte à chaud bloquant le chargement ; chaque compte dispose désormais de son cache, vérifié sans redémarrage.
- StoreKit : les4produits Apple chargent et la feuille de connexion s’ouvre ; annulation sans déblocage. Achat/restauration complets en attente de connexion du compte Sandbox. Aucun paiement réel ni droit premium fictif créé en production.
- Limites de preuve : Android relancé au formulaire18+, parcours connectés finaux non vérifiés sur cet émulateur ; aucun test physique55. Audit npm :16avis modérés transitifs, aucun élevé/critique ; garde des liens entrants ajoutée contre le décodage d’URI malformées. D’autres traductions françaises historiques subsistent hors des parcours corrigés.
...
- Validation finale : TypeScript, ESLint,391tests/71suites terminés normalement ; ExpoDoctor21/21 au cours du lot, CNG56/pods réussis, iOS Release Simulator et Android Release56 compilés, tests Gradle réussis. QA native : retours entre onglets/réglages/langues FR/EN, cartes, profil/répertoire/disponibilités et sessions. Le geste manuel de pull n’a pas pu être piloté par l’outil Mac ; la régression de retour vers l’accueil est vérifiée. Pas de parcours connecté Android final ni d’appareil physique final.
- Premier import55 refusé90683 pour NSMotionUsageDescription référencée par Expo Location ; archive corrigée, sans collecte de mouvement ajoutée. Build55 finalement VALID/APP_STORE_ELIGIBLE (2ee3cf31-e701-44c5-ae0e-e4b7ec1319d5), puis build56 VALID/APP_STORE_ELIGIBLE (ceefb343-69f1-46ac-988c-cce2c450f5b7). Test interne IN_BETA_TESTING ; externe READY_FOR_BETA_SUBMISSION. Aucun groupe de testeurs ni examen bêta externe ajouté.
- Code applicatif0bed221fd40244478e868b45f5ce84489a3d5a4b poussé sur origin/main. IPA56 SHA-256 bca97fbbf32611357f2bbc2b127d3bbc01033b0707dd14e286a602c34d9fa735 ; signature stricte, APNs production, Apple Sign In, debugfalse, endpoint production et absence endpoints locaux contrôlés. APK56 SHA-256 cc4de1cb1952da837f71b7f45a7ff7a9645a3050b3e365ccdc571c48631b7fe6, signaturev2 ; aucune publication Google Play.
- Activation autorisée : trois migrations55 appliquées au projet Supabase existant (89 au total), sync-subscription et revenuecat-webhook déployées. Synchronisation authentifiée200, faux JWT/secret401, tentative de grant client403. Aucun déblocage Premium fictif en production ;145morceaux conservés. Advisors sans ERROR, avertissements documentés. Sitece187a4 déployé READY sur dispoapp.net, pagesFR/EN et6routes de production vérifiées.
...
- Profil personnel redessiné depuis le ZIP fourni : identité, édition/aperçu public, statistiques réelles, progression calculée, instruments et styles, démos avec ajout direct, écoles avec statut réel, liens sociaux. Onglets répertoire/disponibilités conservés ; niveaux déb/int/av/pro sur les cartes de l’accueil. Traductions des nouveaux libellés dans les neuf langues.
- Liens HTTP(S), www et YouTube sans protocole cliquables dans les messages privés, groupes, communautés d’écoles et commentaires de morceaux. Ponctuation finale exclue, schémas dangereux et identifiants dans les URL refusés, appui long/réponse préservés.
- Répertoire : suggestion après les morceaux pour les membres ; pochettes conservées, badge iTunes officiel dans le pied de la tuile. Recherche Apple et résolution des liens existantes conservées. Spotify non activé : absence d’identifiants serveur et accès de production à préparer séparément.
- Événements : bordure rouge dans groupes et Sessions jusqu’à consultation de la révision, stockage local séparé par compte. Les modifications de lieu/jour/heure notifient à nouveau à chaque changement, une seule fois par sauvegarde ; titres/rappels/no-op silencieux pour cette alerte. Ouverture directe de l’événement. Notifications de création, rappels et messages antérieures conservées.
- Migration 20260909133000 appliquée au projet Supabase existant après accord explicite de Ludovic. Tests SQL locaux transactionnels versionnés : répétitions, séries, sauvegarde atomique, permissions, RLS et protection de l’adresse. Contrôle distant des fonctions et RLS ; fonction de création inchangée. Advisors sans nouvelle erreur ni nouvel avertissement (table privée sans policy volontairement inaccessible).
- SOS : bord droit vert et icône d’invitation pour le créateur lorsqu’un musicien compatible couvre un poste ouvert ; actualisation à l’écran. Après confirmation serveur d’une demande directe, ticket déchiré, confettis depuis le bas et retour vers Sessions ; échec conserve la demande, mode mouvement réduit prévu. Haptique iOS succès puis impact léger intégrée.
- Validation : TypeScript, ESLint, 418 tests / 78 suites, formatage ciblé et diff-check ; Expo Doctor 21/21, CNG et pods réussis, builds Release iOS Simulator et Android avec tests Gradle. Parcours connectés isolés localement : profil FR sombre/iOS et Android, aperçu/édition/ajout vidéo iOS, liens privés/groupes/commentaires Android, suggestion sous les morceaux et badge/pochette Android, match SOS, contour rouge puis lecture Android, acceptation SOS et animation filmées sur les deux plateformes. Aucun ressenti haptique vérifié sur appareil physique ; aucun test QA créé en production.
...
- IPA SHA-256 `3aedd07e9ab6bc8360d0780ff260c247f57ecbd505760db3802fa4837facbdd9` ; APK SHA-256 `24f5118713afe2b8604758415107723300d4868dc1c8ebf4d25be92ca856d391`. Rapport hors Git : Dispo-dist/2.4/RELEASE-build59.md.
...
- Depuis l’affiliation du profil : quitter une école ou confirmer son départ puis choisir une autre école dans l’annuaire. Les autres affiliations et le compte restent conservés ; les listes écoles/profils/SOS sont actualisées. Aucune nouvelle migration nécessaire.
- Badge iTunes officiel réduit proportionnellement à24points dans les listes, marge3 et cible44 ; fiche détaillée inchangée, lien vers l’enregistrement conservé.
...
- Clôture build 61 : TypeScript/ESLint, 424 tests/79 suites, formats ciblés, Expo Doctor21/21, CNG/pods ; builds Release iOS et Android arm64 et tests Gradle réussis. Parcours connectés locaux sur les deux plateformes : ajout, déplacement, retrait, réouverture et ordre partagé ; quatre parcours XCTest iOS, relecture par le second compte, passage autonome dans une setlist. Droits et compatibilité de fusion testés, advisors locaux sans problème. Logo exact inspecté sur les deux plateformes.
...
- Version 2.4.1 préparée dans App Store Connect ; restriction volontaire EIGHTEEN_PLUS remplacée par THIRTEEN_PLUS dans le brouillon. La classification calculée pour CHE est THIRTEEN_PLUS, avec messagerie/contenu utilisateur/réseau social toujours déclarés. La version publique 2.4 conserve sa fiche jusqu’à validation Apple.
- Quatre visuels marketing générés depuis des captures natives fictives, en FR/EN, exportés en RGB opaque 1320 × 2868. Ils précèdent une capture native des disponibilités, cinq images COMPLETE par langue dans le brouillon. Assets et prompts hors Git : Dispo-dist/appstore/marketing-20260912/.
- TypeScript, ESLint et 79 suites / 424 tests réussis ; Expo Doctor 20/21, 29 mises à jour patch suggérées pour les dépendances préexistantes, laissées hors de cette modification de texte. Android Release et tests natifs réussis ; APK identité/signature/configuration production contrôlées, texte 13 vérifié dans l’interface native anglaise. Validation iOS et livraison documentées dans Dispo-dist/qa-20260912-age13/ et Dispo-dist/2.4.1/.
- Pages communauté/confidentialité FR/EN alignées dans la branche du site codex/age13-appstore ; build statique, TypeScript, HTML et aperçu local vérifiés. Site migré sur Railway ; ancienne liaison Vercel obsolète, aucune nouvelle infrastructure créée. Déploiement des textes du site et soumission App Review restent à coordonner avant publication publique.
...
- Contrôles natifs : aperçu/liste des collaborateurs et ouverture du répertoire public vus sur iOS/Android ; retrait puis déclaration de collaboration réussis sur les deux plateformes ; répertoire privé conservant le score et le compte sans titres vérifié sur Android. Création d'un groupe avec/sans copie commune, sélection catalogue avec enregistrement et recours manuel vérifiés sur Android. Démarrage du build simulateur de production confirmé sous iOS 27, processus restant actif.
- GitHub : implementation `c71b38a` poussée sur `main`. Apple : build 69 importé `VALID` / `APP_STORE_ELIGIBLE`, TestFlight interne `IN_BETA_TESTING`. Version App Store 2.5.4 soumise le 21 septembre 2026 à 18:55:11 UTC, état `WAITING_FOR_REVIEW`, publication `AFTER_APPROVAL`. Aucun testeur ajouté ni distribution Google Play déclenchée.
- Site FR/EN : informations sur les répertoires/collaborations/confidentialité actualisées et redirections canoniques corrigées, commits `7fc10ea` et `abaa291`, Railway en production vérifié. APK signé build 69 diffusé par SwissTransfer (expiration le 6 octobre 2026).
